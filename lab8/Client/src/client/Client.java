...
public class Client {
    ...
    public String receivedObjectFromServer() {
        try {
            flag = false;
            while (true) {
                ds.setSoTimeout(500);
                ...
                try {
                    ...
                    return "⚠\uFE0F От сервера не поступало никаких ответов. ⚠\uFE0F";
                }
            }
    ...
    public String returnReceivedData () {
        try {
            flag = true;
            String receivedMessage = new String(receivedData, 0, responsePacket.getLength()); // Преобразование в строку
    ...
    public <T> T receiveCollectionFromServer() {
        try {
            flag = false;
            while (true) {
                ds.setSoTimeout(500);
