            ...
            while ((c = bufferedInputStream.read()) != -1) {
                if (c == '\r') {
                    ...
                    if (line.startsWith("execute_script")) {
                        ...
                        Route route = collectionCommands.Commands.readFile(fileInputStream, bufferedInputStream, currentLine, line.startsWith("update_id"), line);
                        if (route == null) {
                            ...
                            if (line.trim().equals("insert")) {
                                ...
                                client.sendObjectToServer(new RemoveGreaterCommand(null));
                                client.receivedObjectFromServer();
                            } else if (line.equals("remove_lower")) {
                                client.sendObjectToServer(new RemoveLowerCommand(null));
                                client.receivedObjectFromServer();
                            } else if (line.startsWith("update_id")) {
                                client.sendObjectToServer(route);
                                client.sendObjectToServer(new UpdateIdCommand(commandData, null));
                                client.receivedObjectFromServer();
                            }
                        }
                    }
